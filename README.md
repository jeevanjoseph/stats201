...
## PROC SGPLOT - Visualize realtionships b/w cat.Predictors vs. cont.Response
...
## Pearson Correlation - Detect degree of linear realtionship between variables  
...
## Multiple Linear Regression - Analyz the effect of multiple cont.Predictors on a cont.Response
...
Common Model selection approaches :
    ...
    - Step-wise selection
        - does not compute all possible models, and is faster and more resonable for very large number of predictors.
        ...
        - Step-wise - starts with no predictors like the forward selection.
            - New predictors are added at every step by checking thier F-statistic of the vars not in the model.
...
## Information Criterion - comparative model evaluation (not absolute).
...
The pattern in the residual plots indicate the assumption that is being violated.
    ...
    - Funnel  shape -> a`ssumption of equal variances` of the error terms are violatied
...
Both `PROC GLMSELECT` as well as `PROC REG` produces the effect plots that include the following plots.
    ...
    - __Influential Observations - CooksD, Rstudent, DFFits.__
        ...
        - DFFits
            ...
            - It generates the predicted value with two data models, teh firat with all obs, and the second with that obs removed
...
Collinearity or AutoCollinearity is when the predictors are closely related. This does not violate any test assumptions, however it leads to biases and incorrect P-values.
    ...
    - `PROC REG` with the VIF option will report the variance inflation or `VIF` along with the parameter estimates. __If the VIF for any term is greater than 10, then the term is likely involved in collinearity.__
